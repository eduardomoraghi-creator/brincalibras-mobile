...
export const usePerfil = () => {
  ...
  const enviarFeedback = useCallback(async (): Promise<ResultadoFeedback> => {
    resetarErro();

    const texto = feedback.trim();

    if (!texto) {
      setFeedbackErro("Digite uma mensagem antes de enviar");
      return { sucesso: false };
    }

    if (texto.length < 3) {
      setFeedbackErro("Feedback muito curto");
      return { sucesso: false };
    }
