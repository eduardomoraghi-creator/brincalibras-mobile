...
function resolvePerfilTheme(appTheme: any): PerfilTheme {
  const isDark =
    appTheme?.dark === true ||
    appTheme?.isDark === true ||
    appTheme?.mode === "dark" ||
    appTheme?.background === "#111418" ||
  ...
  return {
    primary: appTheme?.primary ?? base.primary,
    background: appTheme?.background ?? base.background,
    card: appTheme?.card ?? base.card,
    border: appTheme?.border ?? appTheme?.cardBorder ?? base.border,
    darkButton: appTheme?.darkButton ?? appTheme?.button ?? base.darkButton,
    danger: appTheme?.danger ?? base.danger,
    text: appTheme?.text ?? base.text,
    muted: appTheme?.muted ?? base.muted,
    inputText: appTheme?.inputText ?? base.inputText,
    inputPlaceholder: appTheme?.inputPlaceholder ?? base.inputPlaceholder,
    progressBg: appTheme?.progressBg ?? base.progressBg,
    solidFieldBg: appTheme?.solidFieldBg ?? base.solidFieldBg,
    solidFieldBorder: appTheme?.solidFieldBorder ?? base.solidFieldBorder,
    solidFieldText: appTheme?.solidFieldText ?? base.solidFieldText,
  };
}
...
export default function PerfilScreen() {
  ...
  const perfilTheme = resolvePerfilTheme(theme);
    ...
    return (
      <SafeAreaView
        style={[
          ...
          { backgroundColor: perfilTheme.background },
        ]}
      >
        <ActivityIndicator size="large" color={perfilTheme.primary} />
      </SafeAreaView>
    );
  }
  ...
  return (
    <SafeAreaView
      style={[styles.safeArea, { backgroundColor: perfilTheme.background }]}
    >
      <KeyboardAvoidingView
        ...
        >
          ...
          <TouchableOpacity
            style={[
              styles.medalhasCard,
              {
                backgroundColor: perfilTheme.card,
                borderColor: perfilTheme.border,
              },
            ]}
          ...
          >
            <View style={styles.sectionHeader}>
              <Ionicons name="ribbon" size={18} color={perfilTheme.primary} />
              <Text style={[styles.sectionTitle, { color: perfilTheme.text }]}>
                Medalhas
              </Text>
            </View>
            ...
            >
              <TouchableOpacity
                style={[
                  styles.medalhasOverlayButton,
                  { backgroundColor: perfilTheme.darkButton },
                ]}
                onPress={irParaMedalhas}
                activeOpacity={0.85}
              >
          ...
          <TouchableOpacity
            style={[
              styles.deleteButton,
              { backgroundColor: perfilTheme.danger },
            ]}
